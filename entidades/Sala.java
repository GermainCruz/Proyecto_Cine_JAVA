...
public class Sala {
    ...
    public Sala(int filas, int columnas, DBManager dbManager) {
        ...
        dbManager.inicializarSala(filas, columnas); // Inicializa la base de datos
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (fila < filas && columna < columnas) {
                    if (!disponible) {
                        butacas[fila][columna].ocupar(); // Marcar como ocupada
                    }
                }
            }
    ...
    public void liberarButaca(int fila, int columna) {
        if (!isDisponible(fila, columna)) {
            butacas[fila][columna].liberar();
            dbManager.actualizarButaca(fila, columna, true); // Actualizar en la base de datos
        }
    }
